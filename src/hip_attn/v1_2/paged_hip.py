...
def _forward_delta_attn(
    ...
    is_decode: bool,
    delta_config=None,
    # Legacy kwargs kept for backward compatibility -- ignored if delta_config is set
    ...
    if delta_config is None:
        ...
        delta_config = DeltaAttentionConfig(
            enabled=True,
            smooth=delta_attention_args_smooth,
            just_return=delta_attention_args_just_return,
            base_window_size=delta_attention_args_window,
            diff_mode=delta_attention_args_diff,
            dense_decode=delta_attention_args_dense_decode,
            gamma=delta_attention_args_w,
            exp_enabled=delta_attention_args_exp,
            exp_w=delta_attention_args_exp_w,
            exp_window=delta_attention_args_exp_window,
            exp_sink=delta_attention_args_exp_sink,
            iter_corr=delta_attention_args_iter_corr,
            adjust_norm_const=delta_attention_args_adjust_norm_const,
            extend_mode=delta_attention_args_extend,
        )
        delta_config._parse_bsa_env_vars()

    from hip_attn.v1_2.pipeline.delta_pipeline import DeltaPipeline
    pipeline = DeltaPipeline(delta_config)
...
# Extracted to pipeline/partial_fa3.py -- re-exported for backward compatibility
from hip_attn.v1_2.pipeline.partial_fa3 import _forward_partial_fa3  # noqa: F401
...
def _forward_paged_hip(
    ...
    if using_delta_attention and (get_local_rank() == 0) and (not is_decode):
        info_msg = (
            f"Delta Attention is activated "
            f"base_window_size={delta_config.base_window_size} "
            f"diff_mode={delta_config.diff_mode} "
            f"gamma={delta_config.gamma} "
            f"just_return={delta_config.just_return} "
            f"smooth={delta_config.smooth} "
            f"dense_decode={delta_config.dense_decode} "
            f"exp_enabled={delta_config.exp_enabled} "
            f"exp_w={delta_config.exp_w} "
            f"extend_mode={delta_config.extend_mode} "
        )
